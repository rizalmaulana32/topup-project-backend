...
| `GET` | `/topup/products/:id/payment-methods` | List Duitku payment methods and their fees for that product's selling price, before checkout — amount is looked up server-side, never trusted from the client |
...
| `GET` | `/affiliate/dashboard` | affiliate | Referral stats, commission balance, commission history, and current bank details |
| `PATCH` | `/affiliate/bank-details` | affiliate | Add or update your own payout bank details. Body: `{ bank_name?, account_number?, account_holder? }` — partial update, own profile only |
...
| `GET` | `/admin/withdrawals` | List withdrawal requests. Filter with `?status=pending\|approved\|rejected\|paid\|failed` (`success` is accepted as an alias for `paid`) |
...
| `GET` | `/admin/platform/withdrawals` | List platform withdrawal requests. Filter with `?status=pending\|rejected\|paid\|failed` (`success` is accepted as an alias for `paid`) |
...
- **`GET /topup/products/:id/payment-methods`'s fees always show `0` on this sandbox account.** The endpoint itself is real and working (`DuitkuService.getPaymentMethods`, Duitku's real "Get Payment Method" API) — every payment channel comes back correctly, but Duitku's sandbox doesn't simulate real fee variation per channel/amount. `checkTransactionStatus` (below) proves fees are real on this account (a genuine `"5000.00"` came back for an actually-paid transaction) — real, varying pre-checkout fees should appear once production credentials are used.
- **A second real bug surfaced in production use (2026-08-31)**: a completely normal MomoLive rejection (e.g. a customer typing a game/user ID MomoLive doesn't recognize) was coming back as a `502` instead of a `400` — `MomoProviderTopUpService` was throwing `BadGatewayException` for MomoLive's own `"code": 400` business rejections, the same exception class used for genuine transport failures. Worse, Cloudflare hides the real message behind its own generic error page for any 5xx from the origin, so this looked like a full outage instead of a normal "user not found." Fixed: `BadGatewayException` is now reserved for actual `!response.ok` transport failures; MomoLive's own `code: 400` rejections and invalid (non-numeric) target IDs now correctly surface as `BadRequestException` (400).
...
- **The platform's own withdrawal (`/admin/platform/withdraw` → approve) reuses the same `DuitkuService.createPayout` as affiliate withdrawals**, so it's blocked by the identical disbursement-provisioning issue — it can lock/refund balance correctly, but the actual Duitku call will fail until disbursement is activated. `platform_balance` itself now correctly subtracts Duitku's own transaction fee on every credit (client-reported 2026-09-08 that it didn't match Duitku's real numbers — it was overstating revenue by Duitku's cut on every transaction; fixed via a live `checkTransactionStatus` lookup at credit time). Credits made before that fix were not retroactively corrected.
...
- **Duitku disbursement needs a real bank-code mapping before it can work at all.** Unlike Xendit's derivable `ID_<BANK_NAME>` convention, Duitku uses its own bank code list; `DuitkuService` deliberately throws a clear error instead of guessing at one — this needs to be sourced from Duitku's real bank list once an account exists. This missing mapping used to crash withdrawal approval with a raw `500` (client-reported 2026-09-08, the first time a real withdrawal was actually approved) — `createPayout` now catches it and returns a structured failure like every other Duitku rejection, so approval correctly resolves to `FAILED` with a refund instead of crashing. It still can't actually pay out until the real mapping exists (on top of disbursement itself being unprovisioned).
