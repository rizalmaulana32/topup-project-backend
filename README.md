...
- **Duitku disbursement (affiliate payouts) is not usable yet.** The account's disbursement feature isn't provisioned — real test calls to `/inquiry`, `/transfer`, and the undocumented `/listBank` endpoint (found via an open-source Go SDK, not Duitku's own docs — their official PHP SDK doesn't implement disbursement at all) all return a generic account-level rejection regardless of input, even with a deliberately wrong signature or a garbage email. `/listBank` in particular needs the real email tied to the account's disbursement setup (currently unknown) before it'll return the live bank-code list — once that's available, this is the way to get real bank codes instead of guessing. `DuitkuService.createPayout` implements the two-call inquiry+transfer flow but none of it can be confirmed correct until disbursement is actually activated.
